...
#sys.path.append(str(currentdir) + "/../../src/simple_reordermodel")
...
class OcrInferencer:
    ...
    def __init__(self, cfg):
        ...
        with open("simple_reading_order_model.joblib", mode="rb") as f:
            self.reordermodel=joblib.load(f)
    ...
    def _sort_result_txt(self, xml_data):
        for page_xml in xml_data.iter('PAGE'):
            width=int(page_xml.attrib['WIDTH'])
            height=int(page_xml.attrib['HEIGHT'])
            #print(width,height)
            tmpcoordlist=[]
            for line_xml in page_xml.iter('LINE'):
                ...
                xmin=int(line_xml.attrib['X'])
                ymin=int(line_xml.attrib['Y'])
                xmax=xmin+lw
                ymax=ymin+lh
                verticalflag=0
                if lh>lw:
                    verticalflag=1
                tmpcoordlist.append([xmin,ymin,xmax,ymax,verticalflag,text])
            coordlist=[]
            for t1 in tmpcoordlist:
                xmin1,ymin1,xmax1,ymax1,verticalflag1,text1=t1
                lxmargin,rxmargin,tymargin,bymargin=[2,2,2,2]
                lxcnt,rxcnt,tycnt,bycnt=[0,0,0,0]
                for t2 in tmpcoordlist:
                    xmin2,ymin2,xmax2,ymax2,verticalflag2,text2=t2
                    if t1==t2:
                        continue
                    if xmin1>xmax2:
                        lxcnt+=1
                ...
                coordlist.append([xmin1/width,ymin1/height,(xmin1+xmax1)/(width*2),
                                (ymin1+ymax1)/(height*2),(xmax1-xmin1)/width,(ymax1-ymin1)/height,
                                lxmargin,rxmargin,tymargin,bymargin,
                                lxcnt,rxcnt,tycnt,bycnt,verticalflag1,text1])
            coorddf = pd.DataFrame(coordlist, columns = ['xminp', 'yminp','xcp', 'ycp','widthp','heightp',"lxmargin","rxmargin","tymargin","bymargin","lxcnt","rxcnt","tycnt","bycnt","verticalflag","text"])
            featurelist=['xminp', 'yminp','widthp','heightp',"lxmargin","rxmargin","tymargin","bymargin","lxcnt","rxcnt","tycnt","bycnt","verticalflag"]
            X = coorddf[featurelist]
            if X.shape[0]==0:
                return "",""
            res=self.reordermodel.predict(X)
            coorddf["order"]=res
            coorddf = coorddf.sort_values(['order'],ascending=False)
            print(coorddf[["text","order"]])
            return "\n".join(coorddf["text"].tolist()),""
